function [p,ld_exchange, swap_count, ld_orig] = exchange_sensors4(p,x,sig_n,sig_f,ls,f, eps)
% Function that takes initial placements and improves selection by improving
% the determinant by a factor f and via stochastic greedy algorithm
arguments (Input)
    p
    x
    sig_n
    ...
    eps (1,1) double = 0.5 % Default value if not provided
end
