function KXX = gaussKern(X_input,sig_f,ls, varargin) 
...
if ~isempty(varargin) 
    ...
    rfcn = @(X_input) pdist2(X_input,X_input2);
    % rfcn = @(X_input) geodist(X_input2,X_input); %input by lat and long
    R = rfcn(X_input);
    KXX = sig_f^2*exp(-0.5*(R/ls).^2);
end