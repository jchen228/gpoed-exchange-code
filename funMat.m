classdef funMat
  ...
  methods
    ...
    function y = diagkern(arg1) % Added function to form diagonal elements of kernel in vector form
        N = max(arg1.Asize);
        if isa(arg1,'funMat') 
             for i = 1:N
                 y(i) = double(1:N == i)*arg1.Afun((double(1:N == i)'));
             end
        end
    end

    function y = rowcolkern(arg1, arg2, arg3) % Added function to extract blocks of kernel
        % arg1 is the funmat
        % arg2 is the desired row indicies
        % arg3 is the desired column indicies
        N = max(arg1.Asize);
        sz1 = [length(arg2) N];
        inds1 = sub2ind(sz1, 1:sz1(1), arg2);
        vec1 = zeros(sz1);
        vec1(inds1) = 1;
        sz2 = [N length(arg3)];
        inds2 = sub2ind(sz2, arg3, 1:sz2(2));
        vec2 = zeros(sz2);
        vec2(inds2) = 1;
        if isa(arg1,'funMat') 
             y = vec1*arg1*vec2;
