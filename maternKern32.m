function KXX = maternKern32(X_input,sig_f,ls,varargin)
% Matern 3/2 kernel, ARD-capable: same calling convention as gaussKern.m
% (drop-in replacement), but ls may be a scalar (isotropic) or a 1xD
% vector (ARD -- one lengthscale per input dimension), matching the
% length-scale convention MATLAB's own fitrgp uses for 'ardmatern32'.
...
KXX = sig_f^2 * (1 + sqrt(3)*R) .* exp(-sqrt(3)*R);
end
